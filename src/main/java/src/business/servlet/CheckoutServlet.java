...
import src.business.invoice.service.InvoiceService;
import src.business.invoice_item.dto.InvoiceItemDto;
import src.business.invoice_item.service.InvoiceItemService;
...
public class CheckoutServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        invoiceDto.setUserId(customerId);
        invoiceDto.setStaffId(staffId);
        invoiceDto.setDiscount(discount);
        invoiceDto.setQty(totalQty);
        invoiceDto.setTotal(totalAmount);
        invoiceDto.setPaid(paid);
        invoiceDto.setBalance(balance);
        invoiceDto.setChange(change);

        int billId = invoiceService.createBill(invoiceDto);
        if (billId <= 0) {
        ...
        boolean itemsOk = items.isEmpty() || invoiceItemService.addBillItems(items);
        if (!itemsOk) {
